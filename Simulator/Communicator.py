...
class Communicator(object):
	...
	def CreateChildProcess(self,Execution_Command,Executable_File,Executable_Val):	
		if platform == "darwin" or platform == "linux" or platform == "linux2":
			self.ChildProcess = Popen ([Execution_Command, Executable_File, Executable_Val], stdin = PIPE, stdout = PIPE, bufsize=0,preexec_fn=os.setsid)	
		else:
			self.ChildProcess = Popen ([Execution_Command, Executable_File, Executable_Val], stdin = PIPE, stdout = PIPE, bufsize=0)
