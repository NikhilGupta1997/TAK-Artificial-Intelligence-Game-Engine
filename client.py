...
class Client(Communicator):
	...
	def getGameTimer(self):
		return self.GAME_TIMER // 10000
	
	def setGameTimer(self,Time_in_Seconds):
		self.GAME_TIMER = Time_in_Seconds * 10000
	...
	def CreateChildProcess(self,Execution_Command,Executable_File,Executable_Val):
		...
		if(self.CheckExeFile(Execution_Command,Executable_File)):
			super(Client,self).CreateChildProcess(Execution_Command,Executable_File,Executable_Val)
		else:
...
def game_loop(game, args):
	...
	if args.exe.endswith('.py'):
		client.CreateChildProcess('python', args.exe, args.val)
	elif args.exe.endswith('.sh'):
		client.CreateChildProcess('sh', args.exe, args.val)
	else:
		client.CreateChildProcess('sh', args.exe, args.val)
...
if __name__ == '__main__':
	...
	parser.add_argument('-val', dest = 'val', type = str, default = '0', help = 'value to pass')
